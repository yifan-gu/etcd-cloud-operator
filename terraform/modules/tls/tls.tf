...
# Certificates for the etcd users in the init-acl.

resource "tls_private_key" "acl_users" {
  count = "${var.enabled == true && var.generate_clients_cert == true ? length(var.init_acl_users) : 0}"
...
resource "tls_cert_request" "acl_users" {
  ...
  key_algorithm   = "${element(tls_private_key.acl_users, count.index).algorithm}"
  private_key_pem = "${element(tls_private_key.acl_users, count.index).private_key_pem}"

  subject {
    common_name  = "${var.init_acl_users[count.index]}"
    organization = "etcd"
  }
}
...
resource "tls_locally_signed_cert" "acl_users" {
  ...
  cert_request_pem      = "${tls_cert_request.acl_users.2.cert_request_pem}"
  ca_key_algorithm      = "${local.ca["alg"]}"
  ca_private_key_pem    = "${local.ca["key"]}"
  ca_cert_pem           = "${local.ca["cert"]}"
  validity_period_hours = 43800
